...
//		NodeInterface stub = Util.getProcessStub(node.getSuccessor().getNodeName(), node.getSuccessor().getPort());
//		NodeInterface succ = stub.getSuccessor();
//		
//		if(Util.computeLogic(key, stub.getNodeID().add(BigInteger.ONE), stub.getSuccessor().getNodeID())) {
//			return stub.getSuccessor();
//		}else{
//			return findHighestPredecessor(key).getSuccessor();
//		}
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		for (NodeInterface nodeInterface : fingerTable) {
			NodeInterface stub = Util.getProcessStub(nodeInterface.getNodeName(), nodeInterface.getPort());

			if (Util.computeLogic(stub.getNodeID(), node.getNodeID().add(BigInteger.ONE),
