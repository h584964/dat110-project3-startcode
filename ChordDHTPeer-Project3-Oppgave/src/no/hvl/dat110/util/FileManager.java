...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		for (int i = 0; i < numReplicas; i++) {
			...
			String replicas = filename + i;

			// hash the replica
			hash = Hash.hashOf(replicas);
	...
	public int distributeReplicastoPeers() throws RemoteException {
		...
		int index = rdn.nextInt(Util.numReplicas - 1);
...
//			BigInteger replica = replicafiles[i];
			...
			if (counter == index) {
				succ.saveFileContent(filename, replicafiles[i], bytesOfFile, true);
			} else {
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for (int i = 0; i < replicafiles.length; i++) {
			...
			NodeInterface succ = chordnode.findSuccessor(replicafiles[i]);
			Message metadata = succ.getFilesMetadata().get(replicafiles[i]); 
			System.out.println("Here: "+metadata.getNodeIP());
